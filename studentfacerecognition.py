...
class Face_Recognition:
    def __init__(self, root):
        ...
        b1 = tkinter.Button(root, text='Start Attendance Manager', command=self.face_recog, activeforeground="#ffffff",
    ...
    def mark_attendance(self, i, r, n, d):
        rn = datetime.now()
        date1 = rn.strftime("%d-%m-%y")

        with open(f"studentattendance/{date1}.csv", "w+", newline="\n") as f:
    ...
    def face_recog(self):
        def draw_boundary(img, classifier, scaleFactor, minNeighbours, color, text, clf):
            ...
            for (x, y, w, h) in features:
                ...
                if confidence > 77:
                    cv2.putText(
                        img, f"ID: {id}", (x, y-70), cv2.FONT_HERSHEY_COMPLEX, 0.8, (255, 255, 255), 1)
                    cv2.putText(
                        img, f"Name: {n}", (x, y-55), cv2.FONT_HERSHEY_COMPLEX, 0.8, (255, 255, 255), 1)
                    cv2.putText(
                        img, f"Department: {d}", (x, y-30), cv2.FONT_HERSHEY_COMPLEX, 0.8, (255, 255, 255), 1)
                    cv2.putText(
                        img, f"Roll Number: {r}", (x, y-5), cv2.FONT_HERSHEY_COMPLEX, 0.8, (255, 255, 255), 1)
                    self.mark_attendance(id, r, n, d)
                else:
                    cv2.rectangle(img, (x, y), (x+w, y+h), (255, 0, 0), 3)
                    cv2.putText(img, f"Unidentified Individual", (x, y-55),
        ...
        # 3 - MI Camera / 2 PC Camera / 1 - DroidCam
        video_cap = cv2.VideoCapture(videosrc)
        while True:
