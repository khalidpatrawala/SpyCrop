...
class Attendance:
    def __init__(self,root):
        ...
        # Labels and Entry
        Rollid =tkinter.Label(left_Subframe,text= "Student Roll ID", bd=0,relief=RIDGE,font = ('calibri', 15,), bg='white', fg="black")
    ...
    def importCSV(self):
        global mydata
        mydata.clear()
        fln=filedialog.askopenfilename(initialdir="studentattendance",title="Open CSV", filetypes=(("CSV File",".csv"),("All Files", "*.*")),parent=self.root)
    ...
    def exportCSV(self):
        try:
            if len(mydata)<1:
                ...
                return False
            fln=filedialog.asksaveasfilename(initialdir="studentattendance",title="Open CSV", filetypes=(("CSV File",".csv"),("All Files", "*.*")),parent=self.root)
