...
class Mask_Recognition:
    ...
    def mark_attendance(self, i, r, n, e, d):
        ...
        with open(f"staffalerts/{date}.csv", "w+", newline="\n") as f:
        ...
        with open(f"staffalertmails/{date}.csv", "w+", newline="\n") as f:
                mydatalist = f.readlines()
                name_list = []
                for line in mydatalist:
                    entry = line.split((","))   
    ...
    def face_recog(self):
        ...
        while True:
            ...
            for (x, y, h, w) in faces: 
                ...
                my_cursor.execute(
                    "SELECT fname, lname FROM `host_table` WHERE personId="+str(id))
                n = my_cursor.fetchone()
                n = " ".join(n)

                my_cursor.execute(
                    "SELECT staffid FROM `host_table` WHERE personId="+str(id))
                r = my_cursor.fetchone()
                r = "+".join(r)

                my_cursor.execute(
                    "SELECT dept FROM `host_table` WHERE personId="+str(id))
                d = my_cursor.fetchone()
                d = "+".join(d)

                my_cursor.execute(
                    "SELECT email FROM `host_table` WHERE personId="+str(id))
